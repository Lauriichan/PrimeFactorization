package org.playuniverse.school.numbertheory.ui.scroll;
...
public class JScrollIndicator extends JLayeredPane {
	...
	private static final long serialVersionUID = -7235392826534670523L;
	
	private static final Color THUMB_COLOR = HexTool.parse("#6A136D");
	private static final Color THUMB_BORDER_COLOR = HexTool.parse("#AF23B4");
	private static final float MAX_ALPHA = 0.6f;
	private static final int THUMB_THICKNESS = 4;
	 ...
	 * whenever the component's contents are larger than the view and scrolling in
	 * underway or the mouse is over the scrollbar position.
	 *
	 * @see #setViewportView
	...
	/**
	 * Creates a <code>JScrollIndicator</code> that displays the view component in a
	 * viewport whose view position can be controlled with a pair of scrollbars. The
	 * scrollbar policies specify when the scrollbars are displayed, For example, if
	 * <code>vsbPolicy</code> is
	 ...
	 * policy settings are listed at {@link #JScrollPane.setVerticalScrollBarPolicy}
	...
	/**
	 * Returns the scroll pane used by this scroll indicator. Use carefully (e.g. to
	 * set unit increments) because not all changes have an effect. You have to
	 * write listeners in this cases (e.g. for changing the scrollbar policy)
	 * 
	 * @return
	 */
	...
	private class FadeAnimation {
		...
		public synchronized void fadeIn(boolean mouseOver) {
			...
			if (!isFadeIn) {
				...
				fadeAnimator = PropertySetter
					.createAnimator(FADE_IN_TIME, scrollUI, "alpha", new KeyFrames(KeyValues.create(scrollUI.alpha, MAX_ALPHA), new KeyTimes(0.0f, 1.0f)));
				fadeAnimator.start();

				if (!mouseOver) {
					fadeOutTimer = new Timer(FADE_IN_TIME + STAY_TIME, new ActionListener() {
						@Override
		...
		public synchronized void fadeOut() {
			...
			if (isFadeIn) {
				isFadeIn = false;
				fadeAnimator = PropertySetter
					.createAnimator(FADE_OUT_TIME, scrollUI, "alpha", new KeyFrames(KeyValues.create(scrollUI.alpha, 0.0f), new KeyTimes(0.0f, 1.0f)));
				fadeAnimator.start();
			}
		}
